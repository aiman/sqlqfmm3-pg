...
CREATE VIEW CH08_Good_Bowlers_TBird
AS
SELECT DISTINCT Bowlers.BowlerID, Concat(Bowlers.BowlerLastName, ', ', Bowlers.BowlerFirstName) AS BowlerFullName
                       FROM          ((Bowlers INNER JOIN
                                              Bowler_Scores ON Bowlers.BowlerID = Bowler_Scores.BowlerID) INNER JOIN
...
CREATE VIEW CH08_Good_Bowlers_Bolero
AS
SELECT DISTINCT Bowlers.BowlerID, Concat(Bowlers.BowlerLastName, ', ', Bowlers.BowlerFirstName) AS BowlerFullName
                            FROM          ((Bowlers INNER JOIN
                                                   Bowler_Scores ON Bowlers.BowlerID = Bowler_Scores.BowlerID) INNER JOIN
...
CREATE VIEW CH08_Good_Bowlers_TBird_And_Bolero
AS
SELECT     CH08_Good_Bowlers_TBird.BowlerFullName
FROM       CH08_Good_Bowlers_TBird
INNER JOIN CH08_Good_Bowlers_Bolero
ON         CH08_Good_Bowlers_TBird.BowlerID = CH08_Good_Bowlers_Bolero.BowlerID; 
...
CREATE VIEW CH09_All_Scores_Over_180
AS
SELECT     Tournaments.TourneyDate, Tournaments.TourneyLocation, Bowler_Scores.MatchID, Bowler_Scores.BowlerID, 
                                                   Bowler_Scores.RawScore
                            FROM          (Bowler_Scores INNER JOIN
...
SELECT   Concat(Bowlers.BowlerLastName, ', ', Bowlers.BowlerFirstName) AS BowlerName, TI.TourneyDate, TI.TourneyLocation, 
                      TI.MatchID, TI.RawScore
FROM         Bowlers LEFT OUTER JOIN CH09_All_Scores_Over_180 AS TI
...
CREATE VIEW CH20_Bowler_Scores
AS
SELECT Bowlers.BowlerID, Bowlers.BowlerLastName, Bowlers.BowlerFirstName, CAST(AVG(Bowler_Scores.RawScore) AS Signed Integer) AS BowlerAverage 
FROM Bowlers
INNER JOIN Bowler_Scores 
...
CREATE VIEW CH20_Bowler_Ratings
AS 
SELECT CH20_Bowler_Scores.BowlerID, CH20_Bowler_Scores.BowlerLastName, CH20_Bowler_Scores.BowlerFirstName, CH20_Bowler_Scores.BowlerAverage, ztblBowlerRatings.BowlerRating
FROM ztblBowlerRatings, CH20_Bowler_Scores
WHERE CH20_Bowler_Scores.BowlerAverage BETWEEN ztblBowlerRatings.BowlerLowAvg
AND ztblBowlerRatings.BowlerHighAvg;
...
CREATE VIEW CH20_Tournament_Week_Schedule_2012
AS 
...
FROM ztblWeeks
WHERE ztblWeeks.WeekStart <= CAST('2012-12-31' AS date) And ztblWeeks.WeekEnd >= CAST('2012-09-01' AS date);
